...
[[guide de validation HC-12|HC12-validation]].
...
The repository includes [`tools/plot_remote_postgres.py`](https://github.com/jgrelet/rpi3-meteo/blob/main/tools/plot_remote_postgres.py), a dedicated script to query a remote PostgreSQL instance and produce time-series plots with `matplotlib` and `numpy`.

...
`rpi3-meteo`, en particulier la base PostgreSQL.
...
```bash
/var/lib/docker/volumes/rpi3-meteo_postgres_data/_data
```
...
```bash
docker volume inspect rpi3-meteo_postgres_data
```
...
```bash
docker exec -it rpi3-meteo-postgres ls -la /var/lib/postgresql/data
```

Depuis un conteneur temporaire, sans acceder directement a `/var/lib/docker` :

```bash
docker run --rm -it -v rpi3-meteo_postgres_data:/data alpine ls -la /data
```
...
```bash
sudo ls -la /var/lib/docker/volumes/rpi3-meteo_postgres_data/_data
```
...
```bash
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "SELECT pg_size_pretty(pg_database_size('rpi3_meteo')) AS database_size;"
```

Taille par table, index inclus :

```bash
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "
SELECT
  relname AS table,
...
```bash
docker run --rm -v rpi3-meteo_postgres_data:/data alpine du -sh /data
```
...
```bash
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo
```

Lister les tables et vues :

```bash
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "\dt"
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "\dv"
```

Afficher la structure d'une table ou d'une vue :

```bash
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "\d sensor_readings"
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "\d hourly_sensor_stats"
```

Compter les messages bruts et les mesures capteurs :

```bash
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "SELECT count(*) AS raw_messages FROM raw_messages;"
docker exec -it rpi3-meteo-postgres psql -U jgrelet -d rpi3_meteo -c "SELECT count(*) AS sensor_readings FROM sensor_readings;"
```
